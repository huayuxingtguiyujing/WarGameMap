    ...
    public class ObjectPool {
        ...
        public GameObject GetObject(GameObject prefabObject, Transform parentTransform, Vector3 position, Quaternion quaternion) {
            ...
            if (pool.ContainsKey(prefabName)) {
                if (pool[prefabName] == null) {
                    ...
                    // 对象池中含有该对象的物体，则取出复用，并移出对象池
                    ObjectPoolMark markObj = pool[prefabName][0];
                    result = markObj.gameObject;
                    pool[prefabName].Remove(markObj);
            ...
            if(result.GetComponent<ObjectPoolMark>() == null) {
        ...
        public void RecycleObject(GameObject obj) {
            if (obj.GetComponent<ObjectPoolMark>() == null) {
                // 未挂接对象池标记 
                Debug.Log("can t recycle go: " + obj.name);
                return;
            }
