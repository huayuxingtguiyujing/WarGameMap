...
// and this shader is only used for show hexmap edit effect
    ...
    Properties
    {
        _HexmapDataTexture ("_HexmapDataTexture", 2D) = "white" {}
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _HexmapDataTexture;
            float4 _HexmapDataTexture_ST;

            v2f vert (appdata v)
            {
                v2f o;
                o.vertex = UnityObjectToClipPos(v.vertex);
                o.uv = TRANSFORM_TEX(v.uv, _HexmapDataTexture);
            ...
            {
                // sample the texture
                fixed4 col = tex2D(_HexmapDataTexture, i.uv);
                // apply fog
